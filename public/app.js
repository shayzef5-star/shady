...
async function loadAdmin(container) {
    ...
    container.innerHTML = `
        ...
        <div class="card">
            ...
            ${activities.length === 0 ? '<p>Нет активностей</p>' : activities.map(act => `
                <div style="border-bottom: 1px solid #e5e7eb; padding: 0.5rem 0;">
                    <strong>${act.title}</strong> (${act.date}) - ${act.points} баллов
                </div>
            `).join('')}
        </div>
    `;
...
window.createActivity = async (e) => {
    e.preventDefault();
    
    const activity = {
        title: document.getElementById('admin-title').value,
        description: document.getElementById('admin-description').value,
        category_id: parseInt(document.getElementById('admin-category').value),
        date: document.getElementById('admin-date').value,
        points: parseInt(document.getElementById('admin-points').value),
        max_participants: parseInt(document.getElementById('admin-max').value),
        requirements: document.getElementById('admin-requirements').value,
